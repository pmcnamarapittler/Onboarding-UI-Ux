        ...
        .overlay(alignment: .bottomTrailing) {
            if currentPage == 0 {
                Button(action: {
                    ...
                    .scaleEffect(nextPulse ? 1.02 : 1.0)
                }
                .padding(.trailing, 20)
...
// MARK: - Parallax Background Helper
struct ParallaxImageBackground: View {
    let imageName: String
    var strength: CGFloat = 10 // bigger = stronger parallax
...
struct WelcomeView: View {
    ...
    var body: some View {
        ...
        .background(
            ParallaxImageBackground(imageName: "welcomeBackground", strength: 12)
        )
        .onAppear {
            showContent = true
        }
...
struct FeatureOneView: View {
    ...
    var body: some View {
        ...
        .background(
            ParallaxImageBackground(imageName: "featureone", strength: 12)
        )
...
struct FeatureTwoView: View {
    ...
    var body: some View {
        ...
        .background(
            ParallaxImageBackground(imageName: "featuretwo", strength: 12)
        )
        .onAppear { showContent = true }
    }
}
...
struct RegistrationView: View {
    ...
    enum Field { case name, username, password }
                ...
                VStack(spacing: 20) {
                        ...
                        TextField("", text: $userData.name, prompt: Text("Full Name").foregroundColor(.white.opacity(0.7)))
                            ...
                            .overlay(
                                RoundedRectangle(cornerRadius: 12)
                                    .stroke(focusedField == .name ? Color.white : Color.white.opacity(0.6), lineWidth: 1.5)
                            )
                            .shadow(color: focusedField == .name ? Color.white.opacity(0.25) : Color.clear, radius: 6, x: 0, y: 0)
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        TextField("", text: $userData.username, prompt: Text("Username").foregroundColor(.white.opacity(0.7)))
                            ...
                            .overlay(
                                RoundedRectangle(cornerRadius: 12)
                                    .stroke(focusedField == .username ? Color.white : Color.white.opacity(0.6), lineWidth: 1.5)
                            )
                            .shadow(color: focusedField == .username ? Color.white.opacity(0.25) : Color.clear, radius: 6, x: 0, y: 0)
                            .animation(.easeInOut(duration: 0.2), value: focusedField)
                            .font(.body)
                            .fontWeight(.bold)
                            .foregroundColor(.white)
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        SecureField("", text: $userData.password, prompt: Text("Password (min 6 characters)").foregroundColor(.white.opacity(0.7)))
                            ...
                            .overlay(
                                RoundedRectangle(cornerRadius: 12)
                                    .stroke(focusedField == .password ? Color.white : Color.white.opacity(0.6), lineWidth: 1.5)
                            )
                            .shadow(color: focusedField == .password ? Color.white.opacity(0.25) : Color.clear, radius: 6, x: 0, y: 0)
                            .animation(.easeInOut(duration: 0.2), value: focusedField)
                            .font(.body)
                            .fontWeight(.bold)
                            .foregroundColor(.white)
...
class UserData: ObservableObject {
    @Published var name: String = "" { didSet { saveDraft() } }
    @Published var username: String = "" { didSet { saveDraft() } }
    @Published var password: String = "" { didSet { saveDraft() } }
    ...
    private func loadFromUserDefaults() {
        ...
        self.username = defaults.string(forKey: "userUsername") ?? ""
        self.password = defaults.string(forKey: "userPassword") ?? ""
    }
